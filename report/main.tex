...
So we developed a simple library that generates a graph. This can be done randomly, using the random model or the Barabasi-Albert model, or as '.gml' file.

After the graph is obtained you can use the available graph class attributes and methods as described below.

We chose Python as our programming language for its simplicity and great comunity in this field.
For parsing the data sets we used the networkx library, only using it to read the  '.gml' files and to receive the list of nodes and their edges.
...
\item [generate\_barabasi\_albert\_graph] graph (number of initial nodes, number of links of each node, number of inserted nodes) \textbf{-} receives
                                                            ...
                                                            nodes in the graph returning the graph object for that parameters using the Barabasi-Albert Model.
\end{description}
